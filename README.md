...
### 4. [Spring Kafka 技能](./spring-kafka-skills.md)

基于 Spring for Apache Kafka 源码分析的核心 API、消息流程与设计原理整理。

**核心内容：**
- KafkaTemplate 发送流程与事务支持
- 消息监听容器（单线程/并发）与 Poll 循环机制
- @KafkaListener 注解驱动模型与参数解析
- 错误处理（DefaultErrorHandler、DeadLetterPublishingRecoverer）
- 非阻塞重试（Retry Topic 模式、@RetryableTopic）
- 请求-回复模式（ReplyingKafkaTemplate）
- 事务管理（KafkaTransactionManager、跨资源事务）
- Kafka Streams 集成（StreamsBuilderFactoryBean）
- 测试支持（@EmbeddedKafka、KafkaTestUtils）
- 关键设计模式与源码路径速查
...
- Spring Framework / Spring Boot / Spring Kafka: Apache License 2.0
